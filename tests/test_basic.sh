...
output=$(python3 -m detect_repo_language --primary-only --with-glyph . 2>&1 || true)
echo "  Output: $output"
[[ ! -z "$output" ]] && echo "  PASS" || echo "  FAIL"
echo ""

echo "✓ Test 3: JSON output"
output=$(python3 -m detect_repo_language --json . 2>&1 | head -1 || true)
[[ "$output" == *"{"* ]] && echo "  Output: $output" && echo "  PASS" || echo "  FAIL: $output"
echo ""

echo "✓ Test 4: Full analysis"
output=$(python3 -m detect_repo_language . 2>&1 | grep -E "^=|Repository" | head -1 || true)
[[ "$output" == *"="* ]] && echo "  Output: $output" && echo "  PASS" || echo "  FAIL: $output"
echo ""
