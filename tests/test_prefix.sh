...
output=$(python3 detect_repo_language.py --primary-only --prefix "Language: " . 2>&1 || true)
echo "  Output: $output"
[[ "$output" == "Language: "* ]] && echo "  PASS" || echo "  FAIL"
echo ""

echo "✓ Test 2: Icon prefix"
output=$(python3 detect_repo_language.py --primary-only --prefix "󱔎 " . 2>&1 || true)
echo "  Output: $output"
[[ "$output" == "󱔎 "* ]] && echo "  PASS" || echo "  FAIL"
echo ""

echo "✓ Test 3: Emoji prefix"
output=$(python3 detect_repo_language.py --primary-only --prefix "📝 " . 2>&1 || true)
echo "  Output: $output"
[[ "$output" == "📝 "* ]] && echo "  PASS" || echo "  FAIL"
echo ""

echo "✓ Test 4: Prefix with glyph"
output=$(python3 detect_repo_language.py --primary-only --prefix ">>> " --with-glyph . 2>&1 || true)
echo "  Output: $output"
[[ "$output" == ">>> "* ]] && echo "  PASS" || echo "  FAIL"
echo ""

echo "✓ Test 5: Prefix without --primary-only (should not affect full output)"
output=$(python3 detect_repo_language.py --prefix "test: " . 2>&1 | grep -E "^=|Repository" | head -1 || true)
